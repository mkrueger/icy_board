...
**PPL: Run PPE** builds the open `.pps` with `pplc` and hands the
...
`ppl.boardConfig` names the `icboard.toml` or the directory holding it;
...
`ppl.runArguments` decides how the board is called. It defaults to
...
```json
"ppl.runArguments": ["--runppe", "Sysop;;PWRD:secret;PPE:${ppe};first;second"]
```
