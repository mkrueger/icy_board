...
hint-type-users=A read-only snapshot collection of every registered user on the board. Read an entry by zero-based index or walk it with `FOREACH`.
...
hint-type-regex-matches=An immutable zero-based collection returned by `REGEX.FindAll`.
hint-member-board-users=The users registered when `Board` was first read, exposed as read-only `USER` snapshots.
hint-member-users-count=The number of registered users in this board snapshot.
...
hint-regex-is-match=Reports whether the text has a match at or after the optional 1-based Unicode character position.
hint-regex-find=Returns the first match at or after the optional 1-based Unicode character position.
hint-regex-find-all=Returns matches at or after an optional start position. A positive limit restricts the result count; zero is unlimited up to 100,000.
hint-regex-replace=Replaces all matches, or at most a positive limit, using `$1` and `$name` capture expansion.
hint-regex-split=Splits text into a dynamic one-dimensional string array. Empty fields are retained and a positive limit leaves the remainder in the final element.
hint-regex-match-success=Whether a match was found.
hint-regex-match-value=The complete matched text.
hint-regex-match-start=The 1-based Unicode character position of the match, or zero when no match was found.
...
hint-regex-match-group-start=Returns the selected capture's 1-based Unicode character position, or zero when it did not participate.
hint-regex-match-group-length=Returns the selected capture's length in Unicode characters.
hint-regex-matches-count=The number of matches in this immutable collection.
hint-regex-matches-get=Returns a match by zero-based index. An out-of-range index returns an unsuccessful match.
hint-string-len=Returns the number of Unicode characters in the string.
hint-string-find=Returns the 1-based position of the first match, optionally at or after a start position; 0 means not found.
hint-string-find-last=Returns the 1-based position of the last match, optionally at or before a start position; 0 means not found.
hint-string-contains=Reports whether the string contains a non-empty search string.
hint-string-starts-with=Reports whether the string starts with the prefix.
hint-string-ends-with=Reports whether the string ends with the suffix.
hint-string-count=Counts non-overlapping occurrences of a non-empty search string.
...
hint-string-split=Splits a string into a dynamic one-dimensional string array. Empty elements are retained; an optional limit keeps the unsplit remainder in the last element.
