...
hint-keyword-onerror=Installs a VM-wide handler for operational errors after the current VM statement. Fatal VM and session failures bypass it; ON ERROR OFF keeps manual Error.Last() checks.
...
hint-temporal-legacy=Explicit conversion to the legacy PPE value. Rejects range or precision loss; empty becomes legacy zero. Valid legacy DATE/TIME values widen automatically in assignments and value arguments, including arrays, and in same-kind scalar comparisons. VAR parameters require identical temporal types. Legacy DATE zero becomes empty; legacy TIME zero becomes nonempty midnight (00:00:00).
...
hint-type-zip=Factory for ZIP archives. Create returns a writer; Finish publishes the completed archive.
hint-type-zip-writer=ZIP archive writer with configurable entry options. Errors are reported through Error.Last().
hint-zip-create=Creates a temporary archive for the destination path. Existing files require overwrite=TRUE; the destination is published by Finish.
hint-zip-valid=True while the writer is open and has not failed or been cancelled.
hint-zip-compression-state=Current compression method or level for subsequent entries.
hint-zip-set-compression=Sets the compression method and optional level for subsequent entries.
hint-zip-set-comment=Sets the archive comment using the selected text encoding.
hint-zip-set-zip64=Selects automatic ZIP64 support or rejects entries requiring ZIP64.
hint-zip-set-permissions=Sets Unix permission bits for subsequent entries. Omit mode to restore defaults.
hint-zip-add-file=Adds a regular source file under the optional relative archive name.
hint-zip-add-directory=Adds a directory entry with a relative archive name.
hint-zip-add-tree=Adds a source directory under an optional archive prefix, optionally including subdirectories. The archive output must be outside the source tree.
hint-zip-add-bytes=Adds binary data under the specified relative archive name.
hint-zip-finish=Finalizes the archive and publishes it at the destination. Returns success; errors update Error.Last().
hint-zip-abort=Discards the temporary archive without publishing it.
hint-zip-method=Deflate compresses entries; Stored writes them without compression.
hint-zip-64-mode=Auto permits ZIP64 when needed; Never rejects sizes or entry counts requiring ZIP64.
hint-zip-encoding=Utf8 or Cp437 encoding for ZIP comment text.
hint-param-zip-path=Destination archive path.
hint-param-zip-overwrite=Whether an existing destination file may be replaced on Finish.
hint-param-zip-level=Compression level for the selected ZIP method.
hint-param-zip-encoding=Text encoding selected from ZipEncoding.
hint-param-zip-source-path=Regular file to add to the archive.
hint-param-zip-entry-name=Relative archive path using forward slashes; absolute paths, traversal and duplicate names are rejected.
hint-param-zip-source-directory=Directory whose entries will be added to the archive.
hint-param-zip-prefix=Optional relative directory prefix inside the archive.
hint-param-zip-recursive=Whether to include subdirectories recursively.
...
hint-type-string-unbounded=Unbounded Unicode text. Starting with PPL 400, `STRING` is no longer limited to 256 characters. Lengths and indices count Unicode code points (scalar values), not bytes, graphemes or terminal cells. No automatic Unicode normalization.
...
hint-type-http-request=Mutable HTTP request. `SetQuery`, `SetHeader`, `SetText`, `SetBytes` and `SetForm` change this request and return whether they succeeded.
...
hint-member-terminal-input=Keyboard, mouse, audio-completion and logical-resize events for the caller. Mouse and physical-key reporting require activation; `EventKind.Resize` does not. Call `Release` to return input to the board.
...
hint-member-terminput-wait=Waits for an event, including `EventKind.Resize` without a keypress. Zero polls; a negative value waits without a timeout; a positive value limits the wait in milliseconds.
...
hint-member-terminput-release=Disables input reporting, discards pending logical-resize notifications and returns keyboard and mouse handling to the board.
...
hint-type-terminfo=Read-only snapshot of cached terminal capabilities and the current logical text dimensions. Stored snapshots do not change.
hint-type-terminput=Keyboard, mouse, audio-completion and logical-resize event control for the current caller.
...
hint-type-event=One keyboard, mouse, queue-overflow, audio-channel or logical-resize event returned by `Terminal.Input`. For `EventKind.Resize`, only `Kind` and `Time` are meaningful; read a fresh `Terminal.Info` for dimensions.
...
hint-type-bulletin=A read-only bulletin snapshot with Number, Valid, Path and HasAccess(). Display an accessible entry with DISPFILE.
hint-type-survey=A read-only survey snapshot with Number, Valid, Path, AnswerFile and HasAccess(). For an accessible entry in Session.Conference.Surveys, QUEST Number runs the questionnaire.
...
hint-member-terminal-info=A fresh read-only snapshot of cached capabilities and current logical `Columns`/`Rows`. Sends no query to the terminal; previously stored `TERMINFO` values remain unchanged.
...
hint-member-conference-collections=Read-only one-dimensional snapshot of this conference's file directories, message areas, doors, bulletins, or surveys.
hint-member-conference-introfile=Configured conference introduction display-file path, or an empty string when none is configured. Check HasAccess() before using DISPFILE; reading this property does not display or read the file.
hint-member-survey-metadata=Read-only zero-based Number, Valid flag, questionnaire Path or answer-file path AnswerFile. Valid means a configured entry, not existing files. QUEST Number uses the current session conference; add one to Number for the native S menu. No answers are read or changed by these properties.
hint-member-survey-access=Checks the current session against this snapshot's conference and survey security rules. FALSE for an invalid entry. Call before QUEST Number for an entry in the current conference; does not check file existence or command-level permissions.
hint-member-conference-newsfile=Configured news display-file path, or an empty string when none is configured. Check the conference's HasAccess() before using DISPFILE; reading this property does not read the file.
hint-member-bulletin-metadata=Read-only zero-based Number, Valid flag, or configured display-file Path. Valid means a configured entry, not that its file exists. Add one to Number for the native BBS bulletin number. Display Path with DISPFILE after HasAccess().
hint-member-bulletin-access=Checks the current session against this snapshot's conference and bulletin security rules. FALSE for an invalid entry. Call before displaying Path with DISPFILE; it does not check file existence or command-level permissions.
...
hint-enum-event-kind-resize=The logical text size changed. Read a fresh `Terminal.Info` for `Columns`/`Rows`; stored snapshots remain unchanged. Only `Kind` and `Time` are meaningful. Rapid changes may coalesce into the latest size; identical sizes do not notify. No input mode or keypress is needed, and resize does not reset idle time. Sources: Telnet NAWS and logical ANSI/board resizing; NAWS is limited to 132 columns and 60 rows.
...
hint-enum-regex-options=Open nominal regex options: `None`, `IgnoreCase`, `MultiLine`, `DotMatchesNewLine`, `IgnoreWhitespace`, `SwapGreed` and `Ascii`. Combine with `|`; test all mask bits with `options.Has(mask)` or `(options & mask) == mask`, and any bits with `(options & mask) != RegexOptions.None`. Unnamed values and unknown bits are preserved; Regex.Compile currently supports bits 0–5 and rejects unsupported bits with ErrKind.Regex / ErrCode.Invalid.
...
hint-param-method=HTTP request method or ZIP compression method, as required by the receiver's signature.
...
hint-param-data=Binary data used by the operation.
...
hint-param-start=Optional zero-based Unicode code point position at which searching begins, not a byte offset.
...
hint-param-user-name=Target account's login name or alias; case-insensitive, with surrounding whitespace ignored.
hint-member-session-request-password-recovery=Requests password recovery for a login name or alias using only the account's saved email address. TRUE means the request was accepted, not that mail was sent; FALSE means recovery is globally disabled. Account exclusions, rate limits and delivery failures are logged, not exposed through Error.Last(). No caller permission or login-phase check, output, disconnect or user switch. Available in PPL 4.00.
hint-member-session-edit-message=Edits and replaces the original stored message after access and conflict checks. Returns the saved MSG, or an invalid MSG on abort. Optional header defaults are passed by value; unknown JAM metadata survives. Errors update Error.Last() and support ON ERROR.
hint-member-session-post-message=Runs normal message entry in the target area, including the editor and saving. Optional header and editable initial text supply defaults. Returns the saved MSG, or an invalid MSG on abort; errors update Error.Last().
hint-member-session-reply-message=Composes and saves a reply in the original message's area, with authorized quote text and reply linkage. Optional header and initial text are editable defaults. Returns the saved MSG, or an invalid MSG on abort; errors update Error.Last().
hint-member-session-reply-header=Returns a local MSGHEADER with the normal reply defaults after access checks. Does not open the editor or save a message. Errors update Error.Last().
hint-type-msgheader=A mutable local message header value: From, To, Subject and IsPrivate. Assigning or passing it copies the value; changes do not write to the message base.
hint-member-msg-header-value=Returns an independent mutable MSGHEADER copy of this message's header. Changes remain local until explicitly passed to a Session message operation.
hint-member-msgheader-field=Editable header default, copied by value. Blank From uses the session identity. Sender changes require permission; message area security still applies.
hint-param-message-area=Destination AREA for interactive message entry; its conference and access rules are used without permanently changing the current session area.
hint-param-message-original=Stored MSG identifying the original message and its area. Stale or unauthorized messages are rejected.
hint-param-message-header=Complete MSGHEADER defaults, not a partial patch. Use original.Header for edits or Session.ReplyHeader(original) for replies. The argument is not modified.
hint-param-message-initial-text=Initial message body that the caller can edit before saving; not an immutable prefix.
...
hint-http-url-encode=Percent-encodes one URL component using RFC 3986 rules, where a space is `%20` and `+` is encoded. Encode single values only, never a whole `name=value&...` string, because separators must stay unencoded.
hint-http-url-decode=Decodes one RFC 3986 URL component. A `+` remains literal; byte sequences that are not valid UTF-8 are replaced rather than reported.
hint-http-form-encode=Encodes one `application/x-www-form-urlencoded` field, where a space is `+`. `SetForm()` already applies this encoding automatically.
hint-http-form-decode=Decodes one `application/x-www-form-urlencoded` field, treating `+` as a space. Byte sequences that are not valid UTF-8 are replaced rather than reported.
hint-http-request-property=Read-only request metadata.
hint-http-request-set-query=Sets one URL query parameter, replacing every existing parameter with the same decoded name. The name and value are encoded automatically using RFC 3986; unrelated parameters and the fragment are preserved. An invalid URL returns `FALSE`, leaves the request unchanged and reports `ErrCode.Invalid`.
...
hint-http-request-set-text=Sets this request's UTF-8 body and optional content type, returning whether it succeeded. GET and HEAD requests return `FALSE`; `Error.Last()` reports the failure. JSON, XML and plain text are sent verbatim and need no URL or form encoding. Prefer `SetForm()` for form fields.
hint-http-request-set-bytes=Sets this request's binary body and optional content type, returning whether it succeeded. The default content type is `application/octet-stream`. GET and HEAD requests return `FALSE`; `Error.Last()` reports the failure.
...
hint-http-response-bytes=Returns the retained response body as `BYTES`, whatever it contains. Use it instead of `Text()` for images, archives and any other binary body.
...
hint-regex-is-match=Reports whether the text has a match at or after the optional zero-based Unicode code point position.
hint-regex-find=Returns the first `RegexMatch` at or after the optional zero-based Unicode code point position.
hint-regex-find-all=Returns a `RegexMatch[]` at or after an optional zero-based start position. A positive limit restricts the result count; zero is unlimited up to 100,000. Negative limits or limits above 100,000 report an error regardless of the start position.
hint-regex-replace=Replaces all matches, or at most a positive limit, using `$1` and `$name` capture expansion.
hint-regex-split=Returns a dynamic array of unbounded STRING values by splitting text. Empty fields are retained and a positive limit leaves the remainder in the final element.
hint-regex-match-success=Whether a match was found.
hint-regex-match-value=The complete matched text.
hint-regex-match-start=The zero-based Unicode code point position of the match, or -1 when no match was found.
hint-regex-match-length=The match length in Unicode code points (scalar values), not bytes or graphemes.
...
hint-regex-match-group-start=Returns the selected capture's zero-based Unicode code point position, or -1 when it did not participate.
hint-regex-match-group-length=Returns the selected capture's length in Unicode code points (scalar values).
hint-string-len=Returns the number of Unicode code points (scalar values) in the string, not bytes, graphemes or terminal cells.
hint-string-find=Returns the zero-based Unicode code point position of the first match, optionally at or after a start position and with a StringComparison; -1 means not found.
hint-string-find-last=Returns the zero-based Unicode code point position of the last match, optionally at or before a start position and with a StringComparison; -1 means not found.
...
hint-string-replace=Replaces all occurrences of a substring and returns an unbounded `STRING`.
hint-string-trim=Removes whitespace, or supplied characters, from both ends and returns an unbounded `STRING`.
hint-string-trim-start=Removes whitespace, or supplied characters, from the start and returns an unbounded `STRING`.
hint-string-trim-end=Removes whitespace, or supplied characters, from the end and returns an unbounded `STRING`.
hint-string-to-upper=Converts the string to uppercase and returns an unbounded `STRING`.
hint-string-to-lower=Converts the string to lowercase and returns an unbounded `STRING`.
hint-string-split=Returns a dynamic array of unbounded STRING values by splitting the string. Empty elements are retained; an optional limit keeps the unsplit remainder in the last element.
hint-string-join=Joins a one-dimensional string array with a separator and returns an unbounded `STRING`.
hint-string-repeat=Repeats a string the requested number of times and returns an unbounded `STRING`.
...
hint-statement-join=Joins a conference using the selection in @1. Pass a number or name, for example `JOIN "7"`, or append Q to quick-join: `JOIN "7;Q"`. A numeric argument is also accepted.
...
hint-function-len=
    ### Returns
    Counts Unicode code points (scalar values) in text, bytes in `BYTES`, or elements in an array. Text length is not a grapheme count or terminal width.
hint-function-lower=
    ### Returns
...
hint-statement-on-error=ON ERROR GOTO label | GOSUB label | Procedure | OFF - installs a VM-wide operational-error handler. The first pending error survives later operations and nested function evaluation until the invoking VM statement finishes; Error.Clear() explicitly cancels it. GOTO disarms itself; returning handlers stay armed without recursive dispatch. Fatal VM and session failures bypass handlers.
