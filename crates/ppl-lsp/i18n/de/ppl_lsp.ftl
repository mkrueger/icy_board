...
hint-type-http-request=Unveränderlicher HTTP-Anfrage-Builder. `SetHeader` und `SetText` liefern jeweils einen aktualisierten Anfragewert.
...
hint-member-event-key=Tastaturdaten: übersetzter Tastencode/Text oder physischer Scancode sowie Druck- und Wiederholungsstatus, abhängig von `Kind`.
hint-member-event-position=Mausposition und Angabe, ob `X`/`Y` Pixel statt Textzellen sind.
hint-member-event-mouse=Mausaktion, geänderte Taste oder horizontaler/vertikaler Mausradwert.
...
hint-member-event-buttons=Gibt an, ob die entsprechende Maustaste beim Erzeugen des Ereignisses gehalten wurde.
hint-member-event-modifiers=Gibt an, ob der entsprechende Tastaturmodifikator aktiv war.
...
hint-http-new=Erzeugt eine unveränderliche `HTTPREQUEST` für `HttpMethod.Get`, `Head` oder `Post` und die angegebene URL. Netzwerkzugriff erfolgt erst beim Aufruf von `Send()`.
...
hint-http-request-set-header=Liefert eine neue Anfrage mit diesem Header. Gesperrte oder ungültige Header melden `ErrCode.Invalid`.
hint-http-request-set-text=Liefert eine neue Anfrage mit UTF-8-Body und optionalem Inhaltstyp. GET- und HEAD-Anfragen erlauben keinen Body.
