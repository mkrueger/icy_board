...
hint-type-http-request=Veränderliche HTTP-Anfrage. `SetHeader` und `SetText` ändern diese Anfrage und geben zurück, ob dies erfolgreich war.
...
hint-http-request-set-text=Setzt den UTF-8-Body und den optionalen Inhaltstyp dieser Anfrage und gibt zurück, ob dies erfolgreich war. GET- und HEAD-Anfragen geben `FALSE` zurück; `Error.Last()` meldet den Fehler.
