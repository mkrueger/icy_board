...
hint-temporal-legacy=Explizite Umwandlung in den Legacy-PPE-Wert. Bereichs- oder Präzisionsverlust wird abgewiesen; leer ergibt die Legacy-Null. Gültige Legacy-DATE/TIME-Werte werden bei Zuweisungen und Wertargumenten einschließlich Arrays sowie bei skalaren Vergleichen gleicher Art automatisch erweitert. VAR-Parameter erfordern identische temporale Typen. Legacy-DATE-Null wird leer; Legacy-TIME-Null wird nicht leere Mitternacht (00:00:00).
...
hint-type-zip=Factory für ZIP-Archive. Create liefert einen Writer; Finish veröffentlicht das fertige Archiv.
hint-type-zip-writer=ZIP-Archiv-Writer mit konfigurierbaren Eintragsoptionen. Fehler werden über Error.Last() gemeldet.
hint-zip-create=Erzeugt ein temporäres Archiv für den Zielpfad. Vorhandene Dateien erfordern overwrite=TRUE; Finish veröffentlicht die Zieldatei.
hint-zip-valid=Wahr, solange der Writer geöffnet ist und weder fehlgeschlagen noch abgebrochen wurde.
hint-zip-compression-state=Aktuelles Kompressionsverfahren bzw. aktuelle Stufe für nachfolgende Einträge.
hint-zip-set-compression=Setzt Kompressionsverfahren und optionale Stufe für nachfolgende Einträge.
hint-zip-set-comment=Setzt den Archivkommentar mit der gewählten Textkodierung.
hint-zip-set-zip64=Wählt automatische ZIP64-Unterstützung oder weist Einträge ab, die ZIP64 benötigen.
hint-zip-set-permissions=Setzt Unix-Berechtigungsbits für nachfolgende Einträge. Ohne mode werden die Standardwerte wiederhergestellt.
hint-zip-add-file=Fügt eine reguläre Quelldatei unter dem optionalen relativen Archivnamen hinzu.
hint-zip-add-directory=Fügt einen Verzeichniseintrag mit relativem Archivnamen hinzu.
hint-zip-add-tree=Fügt ein Quellverzeichnis unter einem optionalen Archivpräfix hinzu, auf Wunsch einschließlich Unterverzeichnissen. Die Archivausgabe muss außerhalb des Quellbaums liegen.
hint-zip-add-bytes=Fügt Binärdaten unter dem angegebenen relativen Archivnamen hinzu.
hint-zip-finish=Schließt das Archiv ab und veröffentlicht es am Zielpfad. Liefert den Erfolgsstatus; Fehler setzen Error.Last().
hint-zip-abort=Verwirft das temporäre Archiv, ohne es zu veröffentlichen.
hint-zip-method=Deflate komprimiert Einträge; Stored speichert sie unkomprimiert.
hint-zip-64-mode=Auto erlaubt ZIP64 bei Bedarf; Never weist Größen oder Eintragszahlen ab, die ZIP64 benötigen.
hint-zip-encoding=Utf8- oder Cp437-Kodierung für ZIP-Kommentartext.
hint-param-zip-path=Zielpfad des Archivs.
hint-param-zip-overwrite=Ob Finish eine vorhandene Zieldatei ersetzen darf.
hint-param-zip-level=Kompressionsstufe für das gewählte ZIP-Verfahren.
hint-param-zip-encoding=Textkodierung aus ZipEncoding.
hint-param-zip-source-path=Reguläre Datei, die dem Archiv hinzugefügt wird.
hint-param-zip-entry-name=Relativer Archivpfad mit Schrägstrichen; absolute Pfade, übergeordnete Verweise und doppelte Namen werden abgewiesen.
hint-param-zip-source-directory=Verzeichnis, dessen Einträge dem Archiv hinzugefügt werden.
hint-param-zip-prefix=Optionales relatives Verzeichnispräfix im Archiv.
hint-param-zip-recursive=Ob Unterverzeichnisse rekursiv eingeschlossen werden.
...
hint-type-string-unbounded=Unbegrenzter Unicode-Text. Ab PPL 400 ist `STRING` nicht mehr auf 256 Zeichen beschränkt. Längen und Indizes zählen Unicode-Codepoints (Skalarwerte), nicht Bytes, Grapheme oder Terminalzellen. Keine automatische Unicode-Normalisierung.
...
hint-member-terminal-input=Tastatur-, Maus-, Audioende- und logische Größenereignisse des Anrufers. Maus- und physische Tastenmeldungen müssen aktiviert werden; `EventKind.Resize` nicht. Mit `Release` die Eingabe an das Board zurückgeben.
...
hint-member-terminput-wait=Wartet auf ein Ereignis, auch `EventKind.Resize` ohne Tastendruck. Null fragt ohne Warten ab, ein negativer Wert wartet ohne Timeout, ein positiver Wert begrenzt die Wartezeit in Millisekunden.
...
hint-member-terminput-release=Deaktiviert Eingabemeldungen, verwirft anstehende logische Größenereignisse und gibt Tastatur und Maus an das Board zurück.
...
hint-type-terminfo=Schreibgeschützte Momentaufnahme der gespeicherten Terminalfähigkeiten und aktuellen logischen Textmaße. Gespeicherte Snapshots ändern sich nicht.
hint-type-terminput=Steuerung der Tastatur-, Maus-, Audioende- und logischen Größenereignisse des aktuellen Anrufers.
...
hint-type-event=Ein Tastatur-, Maus-, Warteschlangenüberlauf-, Audiokanal- oder logisches Größenereignis aus `Terminal.Input`. Bei `EventKind.Resize` sind nur `Kind` und `Time` relevant; Maße aus einem neuen `Terminal.Info` lesen.
...
hint-type-bulletin=Schreibgeschützte Bulletin-Momentaufnahme mit Number, Valid, Path und HasAccess(). Zugängliche Einträge können mit DISPFILE angezeigt werden.
hint-type-survey=Schreibgeschützte Umfrage-Momentaufnahme mit Number, Valid, Path, AnswerFile und HasAccess(). QUEST Number startet einen zugänglichen Fragebogen aus Session.Conference.Surveys.
...
hint-member-terminal-info=Neue schreibgeschützte Momentaufnahme der gespeicherten Fähigkeiten und aktuellen logischen `Columns`/`Rows`. Sendet keine Terminalabfrage; zuvor gespeicherte `TERMINFO`-Werte bleiben unverändert.
...
hint-member-audio-fade=Blendet den Kanal auf `targetVolume` über `durationMs` hinweg. <br><br>**Terminalprotokoll:** sendet `APC SyncTERM:A;Volume;C=channel;V=dB;T=durationMs ST`; Lautstärke 0–100 wird in Dezibel umgerechnet.
...
hint-member-conference-collections=Schreibgeschützte eindimensionale Momentaufnahme der Dateiverzeichnisse, Nachrichtenbereiche, Doors, Bulletins oder Umfragen dieser Konferenz.
hint-member-conference-introfile=Konfigurierter Anzeigedateipfad der Konferenzvorstellung, ohne Konfiguration eine leere Zeichenfolge. Vor DISPFILE HasAccess() prüfen; das Lesen dieser Eigenschaft zeigt und liest keine Datei.
hint-member-survey-metadata=Schreibgeschützte nullbasierte Number, Gültigkeitskennung Valid, Fragebogenpfad Path oder Antwortdateipfad AnswerFile. Valid bedeutet einen konfigurierten Eintrag, nicht vorhandene Dateien. QUEST Number verwendet die aktuelle Sitzungskonferenz; für das native S-Menü eins zu Number addieren. Diese Eigenschaften lesen oder ändern keine Antworten.
hint-member-survey-access=Prüft die aktuelle Sitzung gegen die Konferenz- und Umfrage-Zugriffsregeln dieser Momentaufnahme. FALSE bei ungültigem Eintrag. Vor QUEST Number für einen Eintrag der aktuellen Konferenz aufrufen; prüft weder Dateiexistenz noch Befehlsberechtigungen.
hint-member-conference-newsfile=Konfigurierter Pfad der News-Anzeigedatei, ohne Konfiguration eine leere Zeichenfolge. Vor DISPFILE die Konferenz mit HasAccess() prüfen; das Lesen dieser Eigenschaft liest keine Datei.
hint-member-bulletin-metadata=Schreibgeschützte nullbasierte Number, Gültigkeitskennung Valid oder konfigurierter Anzeigedateipfad Path. Valid bedeutet einen konfigurierten Eintrag, nicht eine vorhandene Datei. Für die native BBS-Bulletinnummer eins zu Number addieren. Path nach HasAccess() mit DISPFILE anzeigen.
hint-member-bulletin-access=Prüft die aktuelle Sitzung gegen die Konferenz- und Bulletin-Zugriffsregeln dieser Momentaufnahme. FALSE bei ungültigem Eintrag. Vor der Anzeige von Path mit DISPFILE aufrufen; prüft weder Dateiexistenz noch Befehlsberechtigungen.
...
hint-enum-event-kind-resize=Die logische Textgröße hat sich geändert. `Columns`/`Rows` aus einem neuen `Terminal.Info` lesen; gespeicherte Snapshots bleiben unverändert. Nur `Kind` und `Time` sind relevant. Schnelle Änderungen können auf die neueste Größe zusammengefasst werden; identische Größen melden nichts. Kein Eingabemodus oder Tastendruck erforderlich; Resize setzt die Idle-Zeit nicht zurück. Quellen: Telnet NAWS und logische ANSI-/Board-Größenänderungen; NAWS ist auf 132 Spalten und 60 Zeilen begrenzt.
...
hint-enum-regex-options=Offene nominale Regex-Optionen: `None`, `IgnoreCase`, `MultiLine`, `DotMatchesNewLine`, `IgnoreWhitespace`, `SwapGreed` und `Ascii`. Mit `|` kombinieren; alle Maskenbits mit `options.Has(mask)` oder `(options & mask) == mask` prüfen, mindestens ein Bit mit `(options & mask) != RegexOptions.None`. Unbenannte Werte und unbekannte Bits bleiben erhalten; Regex.Compile unterstützt derzeit Bits 0–5 und weist nicht unterstützte Bits mit ErrKind.Regex / ErrCode.Invalid ab.
...
hint-param-method=HTTP-Anfragemethode oder ZIP-Kompressionsverfahren gemäß der Signatur des Empfängers.
...
hint-param-start=Optionale nullbasierte Unicode-Codepoint-Position, an der die Suche beginnt, kein Byteoffset.
...
hint-member-session-edit-message=Bearbeitet und ersetzt die ursprüngliche Nachricht nach Zugriffs- und Konfliktprüfung. Liefert das gespeicherte MSG, bei Abbruch ein ungültiges MSG. Optionale Header-Vorgaben werden als Wert übergeben; unbekannte JAM-Metadaten bleiben erhalten. Fehler aktualisieren Error.Last() und unterstützen ON ERROR.
hint-member-session-post-message=Führt die normale Nachrichteneingabe in der Ziel-Area einschließlich Editor und Speichern aus. Optionaler Header und editierbarer Anfangstext liefern Vorgaben. Liefert das gespeicherte MSG, bei Abbruch ein ungültiges MSG; Fehler aktualisieren Error.Last().
hint-member-session-reply-message=Verfasst und speichert eine Antwort in der Area des Originals, mit autorisiertem Zitattext und Antwortverknüpfung. Optionaler Header und Anfangstext sind editierbare Vorgaben. Liefert das gespeicherte MSG, bei Abbruch ein ungültiges MSG; Fehler aktualisieren Error.Last().
hint-member-session-reply-header=Liefert einen lokalen MSGHEADER mit den üblichen Antwortvorgaben nach Zugriffsprüfung. Öffnet keinen Editor und speichert keine Nachricht. Fehler aktualisieren Error.Last().
hint-type-msgheader=Ein veränderbarer lokaler Nachrichtenheader mit From, To, Subject und IsPrivate. Zuweisung und Übergabe kopieren den Wert; Änderungen schreiben nicht in die Nachrichtenbasis.
hint-member-msg-header-value=Liefert eine unabhängige, veränderbare MSGHEADER-Kopie des Headers. Änderungen bleiben lokal, bis sie ausdrücklich einer Session-Nachrichtenoperation übergeben werden.
hint-member-msgheader-field=Editierbare Header-Vorgabe mit Wertsemantik. Leeres From verwendet die Session-Identität. Absenderänderungen benötigen Berechtigung; die Sicherheitsregeln der Area gelten weiterhin.
hint-param-message-area=Ziel-AREA der interaktiven Nachrichteneingabe; Konferenz und Zugriffsregeln gelten ohne dauerhafte Änderung der aktuellen Session-Area.
hint-param-message-original=Gespeichertes MSG, das Originalnachricht und Herkunfts-Area bestimmt. Veraltete oder nicht zugängliche Nachrichten werden abgewiesen.
hint-param-message-header=Vollständige MSGHEADER-Vorgaben, kein partieller Patch. Für Änderungen original.Header verwenden, für Antworten Session.ReplyHeader(original). Das Argument wird nicht verändert.
hint-param-message-initial-text=Anfänglicher Nachrichtentext, den der Benutzer vor dem Speichern bearbeiten kann; kein unveränderliches Präfix.
...
hint-regex-is-match=Prüft ab einer optionalen nullbasierten Unicode-Codepoint-Position auf einen Treffer.
hint-regex-find=Liefert den ersten `RegexMatch` ab einer optionalen nullbasierten Unicode-Codepoint-Position.
...
hint-regex-match-start=Die nullbasierte Unicode-Codepoint-Position des Treffers oder -1 ohne Treffer.
hint-regex-match-length=Die Länge des Treffers in Unicode-Codepoints (Skalarwerten), nicht Bytes oder Graphemen.
...
hint-regex-match-group-start=Liefert die nullbasierte Unicode-Codepoint-Position der Gruppe oder -1, wenn sie nicht beteiligt war.
hint-regex-match-group-length=Liefert die Länge der Gruppe in Unicode-Codepoints (Skalarwerten).
hint-string-len=Gibt die Anzahl der Unicode-Codepoints (Skalarwerte) im String zurück, nicht Bytes, Grapheme oder Terminalzellen.
hint-string-find=Gibt die nullbasierte Unicode-Codepoint-Position des ersten Treffers zurück, optional ab einer Startposition und mit StringComparison; -1 bedeutet nicht gefunden.
hint-string-find-last=Gibt die nullbasierte Unicode-Codepoint-Position des letzten Treffers zurück, optional bis zu einer Startposition und mit StringComparison; -1 bedeutet nicht gefunden.
...
hint-statement-join=Betritt die Konferenz aus der Auswahl in @1. Nummer oder Name angeben, z. B. `JOIN "7"`; mit angehängtem Q für einen Schnellbeitritt: `JOIN "7;Q"`. Ein numerisches Argument wird ebenfalls akzeptiert.
...
hint-function-len=### Rückgabewert
    Zählt Unicode-Codepoints (Skalarwerte) in Text, Bytes in `BYTES` oder Elemente in einem Array. Die Textlänge ist weder eine Graphemzahl noch eine Terminalbreite.
