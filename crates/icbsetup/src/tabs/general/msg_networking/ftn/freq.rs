        ...
        impl Page for $name<'_> {
            fn render(&mut self, frame: &mut Frame, area: Rect) {
                ...
                Block::new()
                    ...
                    .title(Line::from(Span::from(get_text($title)).style(get_tui_theme().dialog_box_title)))
                ...
                if let Some(editor) = &mut self.editor {
                    ...
                    Block::new()
                        .title_alignment(Alignment::Center)
                        .title(Line::from(Span::from(get_text($editor_title)).style(get_tui_theme().dialog_box_title)))
                        .style(get_tui_theme().dialog_box)
                        .padding(Padding::new(2, 2, 1, 1))
                        .borders(Borders::ALL)
            ...
            fn handle_key_press(&mut self, key: KeyEvent) -> PageMessage {
                ...
                match key.code {
                    ...
                    KeyCode::Insert => {
                        self.board.lock().unwrap().ftn.freq.$list.push($new);
                        self.table.content_length += 1;
                    }
                    KeyCode::Delete => {
                        if let Some(selected) = self.table.table_state.selected()
                            && selected < self.board.lock().unwrap().ftn.freq.$list.len()
                        {
                            self.board.lock().unwrap().ftn.freq.$list.remove(selected);
                            self.table.content_length -= 1;
                        }
                    }
                    KeyCode::Enter => {
