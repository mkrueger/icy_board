...
impl Websockets {
    pub fn new(icy_board: Arc<Mutex<IcyBoard>>) -> Self {
            ...
            let entry = vec![
                ...
                ConfigEntry::Item(
                    ListItem::new(
                        ...
                        ListValue::Bool(lock.config.login_server.websocket.is_enabled),
                    )
                    ...
                    .with_update_bool_value(&|board: &Arc<Mutex<IcyBoard>>, value: bool| {
                        board.lock().unwrap().config.login_server.websocket.is_enabled = value;
                    }),
                ),
                ConfigEntry::Item(
                    ListItem::new(
                        get_text("connection_info_port"),
                        ListValue::U32(lock.config.login_server.websocket.port as u32, 0, u16::MAX as u32),
                    )
                    .with_status(&get_text("connection_info_port-status"))
                    .with_label_width(label_width)
                    .with_update_u32_value(&|board: &Arc<Mutex<IcyBoard>>, value: u32| {
                        board.lock().unwrap().config.login_server.websocket.port = value as u16;
                    }),
                ),
                ConfigEntry::Item(
                    ListItem::new(
                        get_text("connection_info_address"),
                        ListValue::Text(60, lock.config.login_server.websocket.address.clone()),
                    )
                    .with_status(&get_text("connection_info_address-status"))
                    .with_label_width(label_width)
                    .with_update_text_value(&|board: &Arc<Mutex<IcyBoard>>, value: String| {
                        board.lock().unwrap().config.login_server.websocket.address = value;
                    }),
                ),
                ConfigEntry::Item(
                    ListItem::new(
                        get_text("connection_info_display_file"),
                        ListValue::Path(lock.config.login_server.websocket.display_file.clone()),
                    )
                    .with_status(&get_text("connection_info_display_file-status"))
                    .with_label_width(label_width)
                    .with_update_path_value(&|board: &Arc<Mutex<IcyBoard>>, value: PathBuf| {
                        board.lock().unwrap().config.login_server.websocket.display_file = value;
                    }),
                ),
            ];
        ...
        Self {
            menu: ICBConfigMenuUI::new(get_text("connection_info_websockets"), menu),
        }
    }
}

impl Page for Websockets {
