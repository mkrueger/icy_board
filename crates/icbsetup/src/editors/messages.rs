...
impl<'a> Editor for MessageAreasEditor<'a> {
    fn render(&mut self, frame: &mut Frame, area: Rect) {
        ...
        let block = Block::new()
            ...
            .title(Title::from(Span::from(" Message Areas ").style(get_tui_theme().content_box_title)))
        ...
        if let Some(edit_config) = &mut self.edit_config {
            ...
            let block = Block::new()
                .title_alignment(Alignment::Center)
                .title(Title::from(Span::from(" Message Area ").style(get_tui_theme().content_box_title)))
                .style(get_tui_theme().content_box)
                .padding(Padding::new(2, 2, 1, 1))
                .borders(Borders::ALL)
                .border_type(BorderType::Double);
