...
impl ListItem {
    ...
    fn new(id: &str, title: String, value: ListValue) -> Self {
        let cursor_pos = match &value {
            ...
            ListValue::File(_, text) => text.len() as u16,
            _ => 0
        };
        Self {
            id: id.to_string(),
...
impl GeneralTab {
    pub fn new(menu: Arc<Menu>) -> Self {
        let items = vec![
            ...
            ListItem::new("display_file", "Display File".to_string(), ListValue::Text(25, menu.display_file.to_string_lossy().to_string())),
            ListItem::new("help_file", "Help File".to_string(), ListValue::File(25, menu.help_file.to_string_lossy().to_string())),
...
impl TabPage for GeneralTab {
    ...
    fn render(&self, area: Rect, buf: &mut Buffer) {
        ...
        self.config.render(area, buf, &self.state);
    }
