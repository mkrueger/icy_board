...
impl PcbBoardCommand {
    pub fn text_search(&mut self, action: &Command) -> Res<()> {
        ...
        self.text_search_in_area(&search_text, area, action)
    }

    fn text_search_in_area(&mut self, search_text: &str, area: usize, action: &Command) -> Res<()> {
            ...
            Ok(message_base) => {
                ...
                while start < message_base.active_messages() {
                    if let Ok(msg) = message_base.read_header(start) {
                        ...
                        if !matches.is_empty() {
                            self.read_message_number(&message_base, &viewer, start, Some(matches))?;
                        }
                    }
                    start += 1;
                }
