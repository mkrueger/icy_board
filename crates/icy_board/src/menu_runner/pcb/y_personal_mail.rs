...
impl PcbBoardCommand {
    pub fn personal_mail(&mut self, action: &Command) -> Res<()> {
        ...
        self.state.display_text(IceText::AbortKeys, display_flags::NEWLINE | display_flags::LFBEFORE)?;

        if scan.quick {
            self.state
                .display_text(IceText::ScanHeader1, display_flags::NEWLINE | display_flags::LFBEFORE)?;
            self.state.display_text(IceText::ScanHeader2, display_flags::NEWLINE)?;
            self.state.display_text(IceText::ScanHeader3, display_flags::NEWLINE)?;
        }
        let mut msgs = 0;
        ...
        if scan.all_conf {
            ...
            for (i, conf) in confs.iter().enumerate() {
                ...
                if !scan.skip_zero || (res.msg_from > 0 || res.msg_to > 0) {
                    self.display_result(i, &conf, &res)?;
            ...
            if !scan.skip_zero || (res.msg_from > 0 || res.msg_to > 0) {
                self.display_result(num, &conf, &res)?;
                msgs += res.msg_from + res.msg_to;
            }
        }
    ...
    fn display_result(&mut self, num: usize, conf: &icy_board_engine::icy_board::conferences::Conference, res: &ScanResult) -> Res<()> {
        self.state.reset_color()?;
        self.state.print(TerminalTarget::Both, &format!("{:>5} ", num))?;
        self.state.print(TerminalTarget::Both, &format!("{}", conf.name))?;
        self.state.set_color(TerminalTarget::Both, IcbColor::Dos(8))?;
        ...
        if res.msg_to > 0 {
            self.state.set_color(TerminalTarget::Both, IcbColor::Dos(15))?;
        } else {
            self.state.reset_color()?;
        }
        self.state.print(TerminalTarget::Both, &format!("{:>6}", res.msg_to))?;
        ...
        self.state.print(TerminalTarget::Both, &format!("{:>6}", res.msg_from))?;
        self.state.new_line()?;
        Ok(())
    }
