        ...
        match message_base.restore_message(number) {
            Ok(_) => {
                log::error!(
                    "Restore message {} ({})",
                    number,
                    message_base.get_filename().display()
                );
                self.state
                    .display_text(IceText::MessageRestored, display_flags::DEFAULT)?;
                self.state
                    .print(TerminalTarget::Both, &format!("{}", number))?;
                self.state.new_line()?;
                self.state.new_line()?;
            }
            Err(err) => {
                ...
                self.state
                    .display_text(IceText::NoSuchMessageNumber, display_flags::DEFAULT)?;
                self.state
                    .print(TerminalTarget::Both, &format!("{}", number))?;
                self.state.new_line()?;
                self.state.new_line()?;
