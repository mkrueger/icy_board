# Typed hotkey action labels. Keys/symbols belong to the Rust registry.
# Legacy whole-footer messages below stay available during caller migration.
hotkey_help = Help
hotkey_back = Back
hotkey_exit = Exit
hotkey_edit = Edit
...
hotkey_select = Select
hotkey_up = Up
hotkey_down = Down
hotkey_move = Move
hotkey_browse = Browse
hotkey_search = Search
hotkey_filter = Filter
hotkey_restore = Restore
hotkey_save = Save
hotkey_apply = Apply
hotkey_cancel = Cancel
hotkey_confirm = Confirm
hotkey_close = Close
hotkey_continue = Continue
hotkey_scroll = Scroll
hotkey_page = Page
hotkey_first = First
hotkey_last = Last
hotkey_import = Import
hotkey_load = Load
hotkey_copy = Copy
hotkey_history = History
hotkey_refresh = Refresh
hotkey_field = Field
hotkey_next_field_or_choice = Next field/open choice
hotkey_run = Run
hotkey_preview = Preview
hotkey_sort = Sort
hotkey_reverse_order = Reverse order
hotkey_keep_filter = Keep filter
hotkey_clear_filter = Clear filter
hotkey_jump = Jump
hotkey_next_style = Next style
hotkey_previous_style = Previous style
hotkey_new_door = New door
hotkey_edit_doors = Edit doors
hotkey_edit_bbslink = Edit BBSLINK
hotkey_edit_events = Edit events
hotkey_areas = Areas
hotkey_monitor = Monitor
hotkey_reset_stats = Reset statistics
hotkey_source = Source
hotkey_follow = Follow
hotkey_warnings_errors = Warnings/errors only
hotkey_pan = Pan
hotkey_filter_context = Filter/context
hotkey_next_match = Next match (wrap)
hotkey_previous_match = Previous match (wrap)
hotkey_request_run = Request run
hotkey_refresh_history = Refresh history
hotkey_older = Older
hotkey_newer = Newer
hotkey_scroll_details = Scroll details
hotkey_output = Output
...
# Call wait screen
call_wait_screen_user_button=User
call_wait_screen_user_button_descr=Open a local user login. Network services keep running.
call_wait_screen_sysop_button=Sysop
call_wait_screen_sysop_button_descr=Log in locally as Sysop. Network services keep running.
call_wait_screen_exit_button=Exit
call_wait_screen_exit_button_descr=Exit IcyBoard and stop its services; does not open a shell.
call_wait_screen_log_button=Log Viewer
call_wait_screen_log_button_descr=Read application and caller logs with follow and search filters.
call_wait_screen_status_button=System Status
call_wait_screen_status_button_descr=Show uptime, actual local listeners and free disk space.
call_wait_screen_event_monitor_button=Event Monitor
call_wait_screen_event_monitor_button_descr=Show timed events, their history and run one now.
...
log_view_title = Log Viewer
log_view_app = Application
log_view_caller = Caller Log
log_view_file = Event output (UTF-8)
log_view_keys = Tab Source  F Follow  E Warn/Error  / Filter  Esc Back
log_view_caller_keys = Tab Source  F Follow  / Search  Esc Back
log_view_file_keys = F Follow  / Search  Esc Back
log_view_navigation = ↑↓ Scroll  PgUp/Dn Page  Home/End First/Last  ←→ Pan
log_view_search_keys = Enter Apply  Esc Cancel
log_view_follow = Follow on
log_view_paused = Paused (frozen snapshot)
log_view_warnings = Warnings/errors only
log_view_all = All levels
log_view_search = Filter
log_view_search_edit = Edit search
log_view_context = Context
log_view_matches = Matches
log_view_mode_keys = T Filter/context  n Next match  N Previous match (wrap)
log_view_tail = Read-only: 256 KiB / 2000 lines / 2048 chars per line; follow every 1s
log_view_size = Size
log_view_mtime = Modified (UTC)
log_view_unknown = Unknown
log_view_metadata_unavailable = File metadata unavailable
log_view_rotated = File replaced/rotated
log_view_truncated = File truncated
log_view_read_error = Cannot read log
log_view_unconfigured = No log path configured.
log_view_regular_only = Only regular files are supported; no symlinks or special files.
log_view_empty = No lines to display with the current filters.
log_view_loading = Loading log…
log_view_redacted = [Web admin token entry hidden]

# Read-only system status; running means local listener state, not reachability
system_status_title = System Status
system_status_keys = ↑↓ Scroll  PgUp/Dn Page  Home/End First/Last  Esc Back
system_status_uptime = Uptime (days hh:mm:ss)
system_status_admission = New logins
system_status_admission_open = Allowed
system_status_admission_closed = Blocked
system_status_nodes = Nodes (active/total)
system_status_root = Board root
system_status_disk = Disk (available/total GiB; % available)
system_status_disk_low = Low disk space
system_status_maintenance = Maintenance
system_status_failure = Runtime error
system_status_request_error = Request error
system_status_online_event = Online event
system_status_listeners = Actual local listeners (not a public reachability check)
system_status_telnet = Telnet
system_status_ssh = SSH
system_status_websocket = Secure WebSocket
system_status_admin = Web admin
system_status_configured = Configured
system_status_running = Running locally
system_status_running_closed = Bound; logins blocked
system_status_state_age = State age
system_status_stopped = Stopped
system_status_not_started = Not started
system_status_enabled = Enabled
system_status_disabled = Disabled
system_status_unavailable = Unavailable
system_status_none = None
system_status_waiting = Waiting
system_status_draining = Waiting for sessions to finish
system_status_stopping = Stopping services
system_status_executing = Running command
system_status_reloading = Reloading board
system_status_reload_failed = Reload failed
system_status_listener_failed = Listener start failed
system_status_restarting = Restarting services
system_status_redacted = [Sensitive details hidden]
system_status_config_sample = Configuration sample
system_status_runtime_sample = Runtime sample
system_status_nodes_sample = Nodes sample
system_status_disk_sample = Disk sample
system_status_fresh = Fresh
system_status_stale = Stale
system_status_busy = Busy
system_status_read_failed = Read failed
system_status_pending = Pending
system_status_never = No successful sample yet
...
external_editor_title = External Message Editor
external_editor_mode = Editor
external_editor_mode-status = Select the full-screen message editor
external_editor_mode-help = Internal preserves the built-in editor. Program runs an executable, Script runs a shell script, DOS runs a DOS editor, and PPE runs a compiled PPL editor. The user's line/full-screen preference still applies.
external_editor_mode_internal = Internal
external_editor_mode_program = Program
external_editor_mode_script = Shell script
external_editor_mode_dos = DOS
external_editor_mode_ppe = PPE
external_editor_path = Program or DOS directory
external_editor_path-status = Executable/script/PPE path, or DOS installation directory
external_editor_path-help = DOS uses a copy of this directory for each invocation. Configure the editor and a FOSSIL startup batch inside it first. Other modes use the specified file.
external_editor_arguments = Arguments / DOS command
external_editor_arguments-status = Arguments; DOS requires the complete command
external_editor_arguments-help = Supported placeholders: { "{work}" } exchange directory, { "{node}" } node, { "{baud}" } speed, { "{time}" } minutes. Quote arguments containing spaces. PPE receives the exchange directory as its first GETTOKEN argument.
external_editor_dropfile = User dropfile
external_editor_dropfile-status = User/session file for the external editor
external_editor_dropfile-help = MSGINF and MSGTMP are always generated. EXITINFO.BBS also supplies DORINFO1.DEF for RA editors. PPE does not need a user dropfile. RESULT.ED is an optional output file.
external_editor_timeout = Maximum runtime (seconds)
external_editor_timeout-status = Maximum editor runtime; limited by remaining session time
external_editor_timeout-help = A timeout or disconnect never sends the draft automatically. Set ICE Edit's Errorlevels option to Yes so abort and save can be distinguished.
external_editor_memory = DOS memory (MB)
external_editor_memory-status = Memory allocated to the DOS editor
external_editor_memory-help = Only used by DOS editors. Native DOS assets must first be prepared with icbsetup dos-image.
...
upload_processing_remove_advertisements-status=Check ad members and description blocks using the selected rules; ZIP comments are independent.
upload_processing_remove_advertisements-help=
    One switch enables byte fingerprints, whole-member text rules and bounded advertising
    blocks in canonical description files. Text rules with report_only only report;
    auto_clean removes the complete matched member, while review requires inspection.
    Description cleanup is internally limited to 8 passes; any remaining matches then require review.
    ZIP comments follow only the independent archive comment mode.
...
upload_processing_file_rules=Ad file rules
upload_processing_file_rules-status=TOML rules for advertising members; empty disables this category.
upload_processing_file_rules-help=
    Select a TOML file with [[fingerprint]] and/or [[text_member_rule]] sections (default: upload_ad_files.toml).
    Leave empty to disable member removal. Relative paths start at the board root;
    spaces and semicolons are literal path characters. Remove advertisements must be enabled.
    Read, parse or selected-pattern errors require upload review, not silent skipping.
    Fingerprints still match hashes and sizes or filename patterns with raw keywords, and remove first.
    Text rules compare complete CP437/UTF-8 texts using literal/regex lines; their default action is report_only.
    Inspect new text rules in upload reports before selecting action = "auto_clean".
    Ambiguous text matches require review. A combined TOML file may be selected for both rule paths.
upload_processing_description_rules=Description ad rules
upload_processing_description_rules-status=TOML rules for description blocks; empty disables this category.
upload_processing_description_rules-help=
    Select a TOML file with [[description_rule]] sections (default: upload_ad_descriptions.toml).
    Leave empty to disable description cleanup. Relative paths start at the board root;
    spaces and semicolons are literal path characters. Remove advertisements must be enabled.
    Read, parse or selected-pattern errors require upload review, not silent skipping.
    Use regex lines or plain-text literal_lines. Test new rules with action = "report_only" before enabling "auto_clean".
    A combined TOML file may be selected for both paths; each uses only its own category.
upload_processing_comment_mode=ZIP comment mode
upload_processing_comment_mode-status=Preserve, remove or replace the ZIP comment independently of advertisement rules.
upload_processing_comment_mode-help=
    Preserve keeps the original ZIP comment bytes. Remove clears the comment.
    Replacement comment uses the following text field; empty text removes the comment.
    Stored replacement text is only used in that mode. Comment changes do not require unconditional ZIP recompression.
    Comments in other formats are not exposed. There is no comment rule file.
upload_processing_comment_preserve=Preserve
upload_processing_comment_remove=Remove
upload_processing_comment_replace=Replacement comment
...
upload_processing_replacement_comment-status=Board text used only in Replacement comment mode.
upload_processing_replacement_comment-help=In Replacement comment mode this text completely replaces the ZIP comment; empty text clears it. Other modes retain the stored text but do not use it.
...
msg_networking_zconnect=ZCONNECT Configuration
zconnect_menu-help=Configure the separate ZCONNECT network, its links and JAM area mappings. This is not a Fido transport.
zconnect_general=General settings
zconnect_general-help=Set your ZCONNECT identity and spool directories. The first change selects zconnect.toml if no configuration file is set. Changes are kept in memory; save the board settings to write them to disk.
zconnect_links=Links
zconnect_links-status=Ins: add; Enter: edit; Del: delete; F2: area mappings; F1: help.
zconnect_links-help=Each link describes a remote system and its login credentials. Insert adds a link and opens its editor; Enter edits the selected link; Delete removes it and all its mappings. F2 opens area mappings from the list or link editor. Esc returns; changes remain in memory until you save board settings.
zconnect_areas=Area mappings
zconnect_areas-status=Map remote board names to local JAM bases; read-only receives only.
zconnect_areas-help=Insert adds a mapping; Enter edits it; Delete removes it. Map the exact remote board name to a local JAM message base path (without .jhr/.jdt extensions). Read-only mappings receive messages but do not export local posts. Esc returns to the link. Save board settings to persist changes.
zconnect_link_editor=ZCONNECT link
zconnect_area_editor=ZCONNECT area mapping
zconnect_form_keys=Up/Down: field  Left/Right: choice  F1: help  Esc: back
zconnect_link_keys=Up/Down: field  Enter: profile  F2: areas  F1: help  Esc: back
zconnect_links_keys=Ins: add  Enter: edit  Del: delete  F2: areas  F1: help  Esc: back
zconnect_areas_keys=Ins: add  Enter: edit  Del: delete  F1: help  Esc: back
zconnect_yes=Yes
zconnect_no=No
zconnect_enabled=Enabled
zconnect_enabled-status=Enable or disable the separate ZCONNECT network.
zconnect_enabled-help=Use Space or Left/Right to toggle. Set a fully qualified local system name and configure links before using the mailer. Enabling also selects zconnect.toml if the configuration path is empty.
zconnect_local_system=Local system (FQDN)
zconnect_local_system-status=Fully qualified local system name, e.g. bbs.example.org.
zconnect_local_system-help=Enter your own system's fully qualified domain name, as agreed with your peer. This identifies your board in ZCONNECT exchanges; it is not an FTN address or a URL.
zconnect_local_user=Local user
zconnect_local_user-status=Local ZCONNECT user identity (default: sysop).
zconnect_local_user-help=Enter the local user name used to identify this system in ZCONNECT exchanges. The default is sysop; remote login credentials are configured separately per link.
zconnect_config_file=Configuration file
zconnect_config_file-status=Separate settings file; an empty value defaults to zconnect.toml.
zconnect_config_file-help=Choose where ZCONNECT settings will be saved. Relative paths are relative to the board root. Clearing this field restores zconnect.toml on edit; it does not discard settings. Save board settings to write the file.
zconnect_inbound=Inbound directory
zconnect_inbound-status=Directory for received ZCONNECT packets.
zconnect_inbound-help=Choose the inbound packet spool. Relative paths are resolved against the board root. Keep it separate from outbound packets and local JAM message bases.
zconnect_outbound=Outbound directory
zconnect_outbound-status=Directory for outgoing ZCONNECT packets.
zconnect_outbound-help=Choose the outbound packet spool. Relative paths are resolved against the board root. Keep it separate from inbound packets and local JAM message bases.
zconnect_id=Link ID
zconnect_id-status=Unique identifier for this remote system.
zconnect_id-help=Enter a unique, nonempty link identifier. Use a stable name to select this peer in the mailer. This is separate from the TCP host and remote login name.
zconnect_host=Host
zconnect_host-status=Remote TCP hostname or IP address, without a URL scheme.
zconnect_host-help=Enter the dial hostname, DNS alias or IP address, without telnet:// or a port. It need not match the peer's SYS display name. Leave empty for offline exchange only.
zconnect_remote_system=Expected peer SYS
zconnect_remote_system-status=Optional expected peer display name; empty disables the name check.
zconnect_remote_system-help=Optionally enter the expected peer SYS display name (up to 255 printable ASCII characters, no line breaks). Matching ignores ASCII case. Empty accepts any nonempty peer SYS. This is not the dial host or cryptographic authentication; use a trusted network or secure tunnel.
zconnect_port=TCP port
zconnect_port-status=Remote TCP port, 1–65535 (default: 23).
zconnect_port-help=Set the peer's TCP listening port. The default is 23. Only values from 1 to 65535 are accepted.
zconnect_username=Username
zconnect_username-status=Remote login account for this link.
zconnect_username-help=Our system/account name assigned by the peer, sent as SYS during negotiation and as JANUS Systemname. Empty uses the local fully qualified system name. This does not change outgoing message addresses.
zconnect_password=Password
zconnect_password-status=Remote login password; masked on screen.
zconnect_password-help=Enter the remote account password. It is masked in the editor but stored in cleartext in the configuration and sent in cleartext over Telnet. Protect the file permissions and use a trusted network or secure tunnel.
zconnect_login=Login profile
zconnect_login-status=Enter: choose zconnect, janus or direct (default: zconnect).
zconnect_login-help=Press Enter, use Up/Down and confirm with Enter to select the profile agreed with your peer: zconnect, janus or direct. Esc cancels the choice. Direct is for a peer that starts the protocol without an interactive login. This is a login profile, not an FTN transport.
zconnect_timeout=Timeout (seconds)
zconnect_timeout-status=Connection/protocol timeout in seconds (1-3600; default: 60).
zconnect_timeout-help=Set the connection/protocol timeout from 1 to 3600 seconds (default: 60). The configured value is used without a 30-second cap. Separate finite login/transfer deadlines and retry limits still apply.
zconnect_remote_board=Remote board
zconnect_remote_board-status=Exact remote board/area name used by this peer.
zconnect_remote_board-help=Enter the remote ZCONNECT board name for this mapping. Use the name agreed with the peer, not a local conference number. Map each remote board only once per link.
zconnect_local_area=Local JAM base
zconnect_local_area-status=Local JAM message base path, without file extensions.
zconnect_local_area-help=Enter the local JAM base path, for example messages/general (not messages/general.jhr). Relative paths are resolved against the board root. This must refer to the intended local message area.
zconnect_read_only=Read-only
zconnect_read_only-status=Receive only; do not export local messages for this mapping.
zconnect_read_only-help=Use Space or Left/Right to toggle. When enabled, messages can be imported from the remote board, but local messages will not be exported to it.
...
event_runtime_next_column = Candidate
event_runtime_candidate = Next schedule candidate
event_runtime_schedule_off = Automatic scheduling is off.
event_runtime_event_off = This event is disabled for automatic scheduling.
event_runtime_invalid = Invalid event settings
event_runtime_no_days = No weekdays selected.
event_runtime_wrong_day = Today is not a selected weekday.
event_runtime_window_ended = Today's latest-start window has ended.
event_runtime_window_waiting = Due maintenance window; waiting (runtime snapshot).
event_runtime_unnamed = Event
...
event_runtime_duration = Duration
event_runtime_elapsed = Elapsed
event_runtime_exit_code = Exit code
event_runtime_timing_hint = Timing starts at the journal's attempted start, not proof of process runtime.
    Pending: elapsed; interrupted/wait error or missing times: unknown.
event_runtime_candidate_hint = Candidate is a future schedule slot, not a guaranteed start or backlog.
    Weekdays and daily start windows apply; no calendar date-range settings exist.
event_runtime_manual = Manual
event_runtime_scheduled = Scheduled
event_runtime_log = Log
event_runtime_log_none = No output log recorded for the selected run.
event_runtime_log_missing = Output log or event_logs directory is missing or unreadable.
event_runtime_log_invalid = Invalid output path; only regular files directly under event_logs are allowed.
