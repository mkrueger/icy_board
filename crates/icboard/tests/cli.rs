...
fn starting_without_a_board_explains_how_to_create_one() {
    ...
    let directory = std::env::temp_dir().join(format!("icboard-missing-{}-{nonce}", std::process::id()));
    fs::create_dir(&directory).unwrap();

    let output = Command::new(env!("CARGO_BIN_EXE_icboard"))
...
fn cli_help_errors_and_version_are_localized() {
    for (locale, help, error) in [("en", "Use the full screen", "error"), ("de", "Vollbild verwenden", "Fehler")] {
        ...
        for flag in ["--full-screen", "--localon", "--ppe", "--runppe", "--key", "--version"] {
