...
fn rendered_lines(output: &str) -> Vec<String> {
    ...
    (0..25)
        .map(|y| (0..80).map(|x| screen.char_at((x, y).into()).ch).collect::<String>())
        .collect()
}
...
fn a_long_filename_keeps_size_date_and_description_in_their_columns() {
    ...
    let name_line = lines.iter().position(|line| line.contains("LONG-FILENAME.ZIP")).expect("filename was not listed");
    let fields: Vec<char> = lines[name_line + 1].chars().collect();

    assert!(fields[..13].iter().all(|ch| *ch == ' '), "size did not start in column 13: {:?}", lines[name_line + 1]);
    assert_eq!(fields[13..21].iter().collect::<String>().trim(), "4 B");
    assert_eq!(fields[21..23].iter().collect::<String>(), "  ");
    assert!(fields[23..31].iter().collect::<String>().contains('/'), "date did not start in column 23: {:?}", lines[name_line + 1]);
