...
"#,
    ...
    /// PCBoard's own local screen ends one row above its two status lines, so the last row
    /// a login display may use has to stay visible.
    #[test]
    fn the_last_local_row_is_rendered_above_the_status_bar() {
        for label in ["Username:", "Benutzername:"] {
            let tui = test_tui();
            let display = format!("\x1b[2J\x1b[HLOGIN\x1b[23;1H{label}");
            ...
            assert_eq!(row(22).trim_end(), label);
    ...
    /// Watching a caller whose screen does not leave room for the bar costs screen rows,
