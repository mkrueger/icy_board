...
pub(crate) fn requires_read_password(header: &JamMessageHeader, may_read_all: bool) -> bool {
    !may_read_all
        ...
        && !header
            .sub_fields
            .iter()
            .any(|field| field.field_type() == SubfieldType::FTSKludge && field.content().as_slice() == SENDER)
}
...
pub(crate) fn may_read_header(header: &JamMessageHeader, user: &str, alias: &str, may_read_all: bool) -> bool {
    ...
    !header.is_private()
        || may_read_all
        || [header.to(), header.from()].into_iter().flatten().any(|name| {
...
pub(crate) fn set_security_kind(header: &mut JamMessageHeader, sender_password: bool) {
    header
        .sub_fields
        .retain(|field| !(field.field_type() == SubfieldType::FTSKludge && field.content().starts_with(b"ICYBOARD-SECURITY:")));
    if sender_password {
